...
integrator_scale = 3e-8   # arbitrary scale factor for integrator (dimensionless)
...
def lab_to_cm(theta_lab_deg, E_lab, m_A, m_a, m_b, m_B, Q):
    """
    Convert lab scattering angle to center-of-mass angle.
...
# reads in BCI info from text file
...
# reads in peak volumes and uncertainties from spreadsheet
def parse_input_peaks(file_path):
    # Load spreadsheet with no headers
...
# cross section calculation function
#def cross_section_calculation(BCI_hits, BCI_scale, volume_list, isotope): #func used to convert mass in ug/cm --> 1/barn
def x_sec_calc(BCI_hits, BCI_scale, volume_list): #func used to convert mass in ug/cm --> 1/barn
    ...
    rho_t_6Lid = (TARGET_6Lid * barn_to_cm2 * avogadro_number)/(MOLAR_MASS_6Lid)
    ...
    for i in BCI_hits:
        ...
        dsigma_domega = (volume_list[j] * 1000)/(N_beam * rho_t_6Lid * solid_angle)  # cross-sec in mb/sr
